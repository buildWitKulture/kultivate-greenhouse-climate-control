    ...
    const unsubscribe = onValue(
      ...
      (snapshot) => {
        try {
          ...
          if (data) {
            const greenhousesArray = Object.entries(data).map(
              ([id, greenhouse]: [string, any]) => ({
                ...greenhouse,
                id,
              })
            );
...
export async function updateGreenhouse(
  greenhouseId: string,
  updates: Partial<GreenhouseData>
) {
  const greenhouseRef = ref(database, `greenhouses/${greenhouseId}`);
...
export async function stopSimulation(greenhouseId: string) {
  ...
  const updates = {
    ...
    // // option 3
    // Reset all actuators
    // "simulation/actuatorStates/fan": false,
    // "simulation/actuatorStates/pump": false,
    // "simulation/actuatorStates/lighting": false,
    // "simulation/actuatorStates/misting": false,
  };
    ...
    const unsubscribe = onValue(
      ...
      (snapshot) => {
        try {
          ...
          if (rawData) {
            const processedData = {
              temperature: Object.values(rawData.temperature || {}).map(
                (item: any) => ({
                  timestamp: new Date(item.timestamp),
                  value: item.value,
                })
              ),
              humidity: Object.values(rawData.humidity || {}).map(
                (item: any) => ({
                  timestamp: new Date(item.timestamp),
                  value: item.value,
                })
